...
def read_openpose_json(openpose_json_dir, smooth=True, *args):
    # openpose output format:
    # [x1,y1,c1,x2,y2,c2,...]
    # ignore confidence score, take x and y [x1,y1,x2,y2,...]

    logger.info("start reading openpose files")
    print(openpose_json_dir)
    #load json files
    json_files = os.listdir(openpose_json_dir)
    ...
    for file_name in json_files:
        ...
        _file = os.path.join(openpose_json_dir, file_name)
    ...
    # create frame blocks
    len_cache = len(cache)
    head_frame_block = [0,1,2,3]
    tail_frame_block = [i for i in range(len_cache-4, len_cache)]
    ...
    for frame, xy in cache.items():
        ...
        for neighbor in range(1,4):
            ...
            if frame in head_frame_block and (frame+neighbor) in cache:
                forward += cache[frame+neighbor]
            # last n frames, get value of xy in negative lookahead frames(current frame - 3)
            elif frame in tail_frame_block and (frame-neighbor) in cache:
