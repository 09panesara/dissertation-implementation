...
def test(paco, models_folder, train_test_dir, override_merged_centers=False):
    if paco:
    ...
    if not os.path.isfile(train_test_dir+'/merged_centers.npz') or override_merged_centers:
        merge_centers(emotions, thresh=0, dir=clusters_dir)
    model_results_path = models_folder + '/model_output.npz'

    hmmModel = HMM(paco=False, train=train, test=test,
    ...
    hmmModel.hmm(override_model=True, override_soft_assign=override_soft_assign, n_components=4)
...
if __name__ =='__main__':
    test(paco=False, models_folder='../models/action_db/output/train-test', train_test_dir='../data/action_db/train-test', override_merged_centers=True)
